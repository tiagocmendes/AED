...
person::person(const char *name,const char *address, const std::string phone)
{ // constructor
  if(debug != 0)
...
int main(void)
{
  person p = person("John Doe","no address", "132341114"); // create a new object and then copy it to p
  person q("no name","no address", "no phone");           // create a new object (no copy needed)

  cout << "Before change of address: " << p << endl;
  p.set_address("bench 24, Central Park, New York");
  cout << "After change of address:  " << p << endl;
